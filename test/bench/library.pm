...
///    page = 4*4096;
///    PersistString::startPager(page, pgs, 0, 0);*/

#ifndef PAGER_START_OBJ
    unsigned page = 16384;        
    Library::startPager(128,2,0,0);
    unsigned pgs = booksCount / BOOKS_TO_AUTHORS_RATIO;    
    Author::startPager(page, pgs, 0, 0);
    pgs = booksCount;
    Book::startPager(page,pgs,0,0);
    pgs = booksCount * MAX_AUTHORS_PER_BOOK / 2.0;
    BookToAuthor::startPager(page, pgs, 0, 0);
    pgs = booksCount == 0 ? 1000 : booksCount/5;       
    PersistString::startPager(page, pgs, 0, 0);
#else
    unsigned objPerPage = 1;
    unsigned maxObjects = 1;
    Library::startPagerObj(objPerPage, maxObjects, maxObjects, 2);

    objPerPage = booksCount / BOOKS_TO_AUTHORS_RATIO;
    ...
    objPerPage = 4096;
    maxObjects = objPerPage*booksCount;
    PersistString::startPagerObj(objPerPage, maxObjects, maxObjects, 2);
#endif
    PersistStart;
...
{    
    ...
    for(unsigned i=0; i<booksCount; i++)
    {
        ...
        books::addTail(lib, book);

#ifndef ABSTRACTS_AFTER_TITLES
        if(genAbstract)
        {
			//MyRand gets number from 0..511
            absSize = MyRand(512)+1;
            absSize = absSize < 16 ? 16 : absSize;
            abstr = new char[absSize];
            memset(abstr, 'a', absSize);
            abstr[absSize-1] = '\0';
            bookAbstract::addNew(book, abstr);
        }
#endif //ABSTRACTS_AFTER_TITLES
        //random authors
...
void OpenLibrary(bool testing)
{
    ...
    {
#ifdef ABSTRACTS_AFTER_TITLES
        if(bookTitle::fwd(book)==NULL)
        {
            //remove abstract if any
            bookAbstract::del(book);            
#else
        if((cnt%4)==0)
        {
            //remove books name            
