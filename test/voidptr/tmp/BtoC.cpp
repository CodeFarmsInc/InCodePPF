...
PTR(C) const BtoC_Ring1::next(PTR(C) tail, PTR(C) c){
    PTR(C) ret=c->ZZds.ZZBtoC.next;
    if(c==tail)ret=NULL;
    return ret; // return NULL when c is the externally given tail
}

PTR(C) const BtoC_Ring1::nextRing(PTR(C) c){
    return c->ZZds.ZZBtoC.next;
}

PTR(C) BtoC_Ring1::addHead(PTR(C) tail, PTR(C) c){
...
PTR(C) BtoC_Ring1::addTail(PTR(C) tail, PTR(C) c){
    if(c->ZZds.ZZBtoC.next){
...
PTR(C) BtoC_Ring1::append(PTR(C) tail,PTR(C) c1, PTR(C) c2){
    PTR(C) x;
...
PTR(C) BtoC_Ring1::remove(PTR(C) tail, PTR(C) c){
    PTR(C) x,nxt;
...
PTR(C) BtoC_Ring1::sort(BtoC_sortFun cmp, PTR(C) tail){
    PTR(C) a1,b1,a2,b2,t1,t2,temp,last,nxt;
    int stopFlg,choice;
...
void BtoC_Ring1::merge(PTR(C) s,PTR(C) t){
    PTR(C) u,v;
...
PTR(C) BtoC_Ring1Iterator::fromHead(PTR(C) p){ 
    PTR(C) ret;
...
PTR(C) const BtoC_Ring1Iterator::next(){ 
    PTR(C) c;
...
void BtoC_Ring1Iterator::start(PTR(C) p){ 
    PTR(C) ret;
...
PTR(C) const BtoC_Ring1Iterator::operator++(){ 
    PTR(C) ret;
