...
// These parameters store index of the position of the invocatin  -*
// in argv[]
...
public:
    ...
    int ppfPath;
    MainParameters(){
        flex=umlRun=dolRun=objcRun=genPath=ppfPath=0;
    }
};
...
class Generator { // root of all the data
    ...
    char *dsFileName,*libPath,*outFileName,*ppfPath,
...
public:
    ZZ_Generator ZZds;
    Generator(MainParameters *mPar,char **argv,char *_ppfPath);
    int generate(char *tmpLibPath);
    int genUmlFile();
