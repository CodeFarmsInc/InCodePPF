...
/****************************************************************/
// Author: Jiri Soukup, Sept.23, 2013 
...
// Syntax:
//    The syntax may have two simple, historical forms (used by most of the
//    regression tests)
//      codegen [-dol] dsFile libPath genFile ... just to compile
//      codegen -uml [-dol] dsFile libPath genFile srcList umlID ... also UML 
//    or a new, more general form
//      codegen -flex dsFile libPath gen File [-uml srcList, umlID] [-dol] 
//                         [-dol] [-objc] [-genLib tmpLibPath] [-ppf ppfPath]
// where:
//    -flex   specifies the new, more flexible syntax
//    -uml    triggers automatic generation of the UML class diagram. 
//    -dol    selects the old DOL interface (for historical comparison)
//    -objc   required when using ObjectiveC language
//    -genLib directory to where codegen can deposit the customized classes
//            (if not used, the default is subdirectory "tmp")
//    -ppf    location of the PPF (Persistent Pointer Factory), c:\ppf
//            by default when -ppf is not used.
//
...
//            project
//    tmpLibPath  path to the directory of customized for this task, it is 'tmp'
//            by default when -genLib is not used
//    ppfPath is the path to the the PPF (Persistent Pointer Factory), it is
//            "c:\ppf" by default, when -ppf is not used.
...
//
// WARNING: The call to 'codegen' is slightly different from Java and,
//          at the moment, only the two simple forms are available in Java.
...
// Ver.3.0 (February 5, 2014)
//    extended 'flex' menu, relative positions of directories
// ----------------------------------------------------------------------------
...
// i.e. when umlRun=1.
// ----------------------------------------------------------------------
Generator::Generator(MainParameters *mp,char **argv,char *_ppfPath){
    int i,k;
    static char* version = "3.1 ObjectiveC capable, tmp and ppf paths selectable";
    ...
    i=mp->flex; // this will work both for flex==0 and flex>0
    if(i>0)i++;
...
// ------------------------------------------------------------------
int Generator::generate(char *tmpLibPath){
    FILE *hFile,*cFile;
    char fName[80];
    ...
    fprintf(hFile,"\n#define PPF_PATH \"%s\\factory.h\"\n",ppfPath);

    fprintf(hFile,"#include \"%s\\general.h\"\n",libPath);
    genClasses(hFile);
    genIncludes(hFile,0,tmpLibPath);
    ...
    if(mPar->objcRun) strcat(fName,".m");
    else        strcat(fName,".cpp");

    cFile=fopen(fName,"w");
    if(!cFile){printf("codegen: problem to open file %s\n",fName); return 1;}

    genIncludes(cFile,1,tmpLibPath);
    fclose(cFile);
    // ------------------------------
    
    genFiles(libPath,tmpLibPath);
    return 0;
}
...
void Generator::genFiles(char *sourcePath,char *targetPath){
    ...
    ITERATE(oIter,org){
        ...
        for(k=0; k<level; k++){
            ...
            strcat(name2,exth);
            substitute(name1,name2,params,numParams,mPar->objcRun);
...
void Generator::genTypedefs(FILE *file){
    ...
    ITERATE(oIter,org){
        ...
        ITERATE(iIter,iter){
            ...
            if(mPar->dolRun && !strcmp(itName,"Iterator")){
...
int Generator::genDefinitions(FILE *file){
    ...
    fprintf(file,"// -----------------------------------------------\n");
    if(mPar->dolRun==1)fprintf(file,"#ifdef ZZmain\n");
    for(i=0; i<=mPar->dolRun; i++){ // for 'dol' style, run this twice
        if(i==1)fprintf(file,"\n#else // ZZmain\n");
        oIter.start(this);
        ITERATE(oIter,org){
            ot=myType::next(org);
            if(mPar->objcRun==1)    fprintf(file,"typedef ");
	    else if(mPar->dolRun==0)fprintf(file,"typedef class ");
            if(i==1)fprintf(file,"extern ");
            fprintf(file,"%s_%s",orgName::get(org),orgTypeName::get(ot));
            fprintf(file," %s;\n",orgName::get(org));
        }
    }
    if(mPar->dolRun==1)fprintf(file,"#endif // ZZmain\n");
    fprintf(file,"// -----------------------------------------------\n");
    fprintf(file,"\n");
    return 0;
}
...
void Generator::genClasses(FILE *file){
    ...
    ITERATE(oIter,org){
        ...
        ITERATE(pIter,part){
            ...
            if(printed==1){
		if(mPar->objcRun) fprintf(file,"@class %s;\n",partName::get(part));
...
// --------------------------------------------------------------------------
// For a description of the syntax, look at the comment part at the beginning
// of this file.
// --------------------------------------------------------------------------
int main(int argc,char **argv){
    int err,i,numOpt,numArg,paramStart; char *tmpLibPath, *_ppfPath;
    ...
    if(argc<4)err=1;
        ...
        for(i=0; i<argc; i++){
            if(!strcmp(argv[i],"-dol")){
                mp->dolRun=i; numArg++; numOpt++;
            }
	    else if(!strcmp(argv[i],"-uml")){
                mp->umlRun=i; numArg=numArg+3; numOpt++;
            }
	    else if(!strcmp(argv[i],"-objc")){
                mp->objcRun=i; numArg++; numOpt++;
            }
	    else if(!strcmp(argv[i],"-genLib")){
                mp->genPath=i; numArg=numArg+2; numOpt++;
            }
	    else if(!strcmp(argv[i],"-ppf")){
                mp->ppfPath=i; numArg=numArg+2; numOpt++;
            }
	    else if(!strcmp(argv[i],"-flex")){
                mp->flex=i; numArg=numArg+4; numOpt++;
            }
        }
    }
	...
	paramStart=numArg;
       	numArg=numArg+3;
	if(mp->dolRun>0)mp->dolRun=1;
	if(mp->umlRun>0)mp->umlRun=1;
	if(mp->objcRun>0)mp->objcRun=1;
	if(mp->genPath>0)mp->genPath=1;
	if(mp->ppfPath>0)mp->ppfPath=1;
	if(mp->flex>0)mp->flex=1;
    }
    else           {paramStart=0;}

    if(numArg != argc)err=1;

    i=mp->ppfPath;
    if(i==0)_ppfPath="c:\\ppf";
    else    _ppfPath=argv[i+1];

    if(err){
        printf("ERROR in calling codegen(), two old styles of use allowed:\n");
        printf("codegen [-dol] [-objc] dsFile libPath genFile\n");
        printf("codegen -uml [-dol] [-objc] dsFile libPath genFile srcList umlID\n");
	printf("   where the order of flags and parameters is fixed\n\n");
	printf("The new style provides more options and the order is flexible:\n");
	printf("codegen [-flex dsFile libPath genFile] [-uml srcList umlID]\n");
	printf("        [-genLib gPath] [-ppf ppfPath]\n");
	printf("   where the -flex option must be used\n");
        return 1;
    }

    Generator generator(mp,&(argv[paramStart]),_ppfPath);
    ...
    i=mp->genPath;
    if(i==0)tmpLibPath="tmp";
    else    tmpLibPath=argv[i+1];
    if(generator.generate(tmpLibPath))return 1;
    return 0;
}
