...
PTR($1) const $$_Ring1::next(PTR($1) tail, PTR($1) c){
    PTR($1) ret=c->$0.next;
    if(c==tail)ret=NULL;
    return ret; // return NULL when c is the externally given tail
}

PTR($1) const $$_Ring1::nextRing(PTR($1) c){
    return c->$0.next;
}

PTR($1) $$_Ring1::addHead(PTR($1) tail, PTR($1) c){
    if(c->$0.next){
...
PTR($1) $$_Ring1::addTail(PTR($1) tail, PTR($1) c){
    if(c->$0.next){
...
PTR($1) $$_Ring1::append(PTR($1) tail,PTR($1) c1, PTR($1) c2){
    PTR($1) x;
...
PTR($1) $$_Ring1::remove(PTR($1) tail, PTR($1) c){
    PTR($1) x,nxt;
...
PTR($1) $$_Ring1::sort($$_sortFun cmp, PTR($1) tail){
    PTR($1) a1,b1,a2,b2,t1,t2,temp,last,nxt;
    int stopFlg,choice;
...
void $$_Ring1::merge(PTR($1) s,PTR($1) t){
    PTR($1) u,v;
...
PTR($1) $$_Ring1Iterator::fromHead(PTR($1) p){ 
    PTR($1) ret;
...
PTR($1) const $$_Ring1Iterator::next(){ 
    PTR($1) c;
...
void $$_Ring1Iterator::start(PTR($1) p){ 
    PTR($1) ret;
...
PTR($1) const $$_Ring1Iterator::operator++(){ 
    PTR($1) ret;
