...
// as NULL. This is necessary, because the constructor of PersistPtr
// is used for the initialization of virt.fun.pointers, and 
// therefore must be empty.
// -------------------------------------------
PTR(PersistVoid) $$_Hash::form(PTR($1) hp,int const sz){
    PTR(PersistVoid) pvPtr;  PersistVoid pvObj; 
    int i;

    pvPtr=$$_Array::form(hp,sz,0); // 0 = fixed array
    if(pvPtr==NULL)return pvPtr;
    for(i=0; i<sz; i++){ 
	pvObj=$$_Array::get(hp,i);
	pvObj=NULL;
	$$_Array::set(hp,i,pvObj);
    }
...
// -------------------------------------------
int $$_Hash::resize(PTR($1) hp, int newSz){
    PTR($2) p,head,nxt;
    $$_Iterator it;
    ...
    if($$_Array::sizeChange(hp,newSz,2))return 1; // 0 for exact size
...
// -------------------------------------------
PTR($2) $$_Hash::get(PTR($1) hp, PTR($2) obj){
    int i; PTR($2) p,s;

    i=hash(obj,hp->$0.sz);
    p=from_void($2,hp->$0.array[i]);
    if(p){
...
// Returns: 0=everything OK, 1=error
// -------------------------------------------
int $$_Hash::add(PTR($1) hp, PTR($2) obj){ // obj = object to load into the hash table
    int i; PTR($2) p;
    ...
    i=hash(obj,hp->$0.sz);
    p=from_void($2,hp->$0.array[i]);
    if(p!=NULL){obj->$0.next=p->$0.next; p->$0.next=obj;}
...
// -------------------------------------------
PTR($2) $$_Hash::remove(PTR($1) hp, PTR($2) obj){
    int i; PTR($2) p,s,prev;
...
// -------------------------------------------------------------
PTR($2) $$_Hash::slot(PTR($1) hp, int i){
    return from_void($2,hp->$0.array[i]);
}

// -----------------------------------------------------------------
int $$_Hash::size(PTR($1) hp, int *popCount){ 
    *popCount=hp->$0.count;
...
// -----------------------------------------------------------------
PTR($2) $$_HashIterator::first(PTR($1) hp,int i){
    PTR($2) p; int sz;
    ...
    for(; bucket<sz; bucket++){
        tail=from_void($2,hp->$0.array[bucket]);
        if(!all || tail)break;
    }
...
PTR($2) const $$_HashIterator::next(){
    PTR($2) p; int sz;
    ...
    if(nxt==tail){ // search for the next non-empty bucket
        if(!all)nxt=NULL; 
            ...
            for( tail=NULL, bucket++; bucket<sz; bucket++){
                tail=from_void($2,holder->$0.array[bucket]);
                if(tail)break;
            }
            if(!tail)nxt=NULL;
...
// -----------------------------------------------------------------
PTR($2) const $$_HashIterator::operator++(){
    PTR($2) ret;
