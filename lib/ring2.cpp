...
int $$_Ring2::debugFun(PTR($1) tail){
    PTR($1) p,nxt,head; int h,t,ret;
    $$_Ring2Iterator it;
...
PTR($1) const $$_Ring2::next(PTR($1) tail, PTR($1) c){
    PTR($1) ret=c->$0.next;
    if(c==tail)ret=NULL;
    return ret; // return NULL when c is the tail
}

PTR($1) const $$_Ring2::prev(PTR($1) tail, PTR($1) c){
    PTR($1) ret=c->$0.prev;
    if(ret==tail)ret=NULL;
    return ret; // return NULL when c is the tail
}

PTR($1) const $$_Ring2::nextRing(PTR($1) c){
    return c->$0.next;
}

PTR($1) const $$_Ring2::prevRing(PTR($1) c){
    return c->$0.prev;
}

PTR($1) $$_Ring2::addHead(PTR($1) tail, PTR($1) c){
    PTR($1) head;
...
PTR($1) $$_Ring2::addTail(PTR($1) tail, PTR($1) c){
    if(c->$0.next){
...
PTR($1) $$_Ring2::append(PTR($1) tail,PTR($1) c1, PTR($1) c2){
    PTR($1) nxt;
...
void $$_Ring2::insert(PTR($1) c1, PTR($1) c2){
    PTR($1) prv;
...
PTR($1) $$_Ring2::remove(PTR($1) tail, PTR($1) c){
    PTR($1) prv,nxt; PTR($1) t;
...
PTR($1) $$_Ring2::sort($$_sortFun cmp, PTR($1) tail){
    PTR($1) a1,a2,t,t1,t2,p,nxt,subs,last,lastA1;
...
void $$_Ring2::merge(PTR($1) s,PTR($1) t){
    PTR($1) u,v;
...
PTR($1) $$_Ring2Iterator::fromHead(PTR($1) p){ 
    PTR($1) ret;
...
PTR($1) $$_Ring2Iterator::fromTail(PTR($1) p){ 
    dir=1;
    if(p==NULL){nxt=tail=NULL; return NULL;}
    tail=p;
    nxt=tail->$0.prev;
...
PTR($1) const $$_Ring2Iterator::next(){ 
    PTR($1) c;
...
void $$_Ring2Iterator::start(PTR($1) p){ 
    PTR($1) ret;
...
PTR($1) const $$_Ring2Iterator::operator++(){ 
    PTR($1) ret;
